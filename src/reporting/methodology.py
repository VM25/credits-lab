...
def build() -> dict:
    ...
    methodology = {
        "data_sources": {
            ...
            "credit_rejects": "LendingClub rejected applicants (observed records) + clearly-labeled synthetic rejects for reject-inference/policy context only (never used for supervised PD training).",
            "card_payments": "Kaggle credit-card fraud (mlg-ulb/creditcardfraud) - observed labels/amount/Time/V1..V28; deterministically downsampled (all fraud kept).",
        ...
        "synthetic_data_disclosure": {
            ...
            "synthetic_features_on_observed_records": [
                "card payments: merchant_category/merchant_risk_band/location_proxy/device_proxy/account_id/account_age_days and 24h velocity are synthetic context attached to observed fraud labels + amount + PCA features"],
