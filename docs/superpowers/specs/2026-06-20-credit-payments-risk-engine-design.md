...
- Source: `wordsforthewise/lending-club` → `accepted_2007_to_2018Q4.csv.gz`. **Seeded, stratified** chunked sample to ~60–80k rows: stratify by issue year (and status where feasible) so the year/status distribution is preserved and time-aware train/validation/test splits remain usable (no year starved of rows). Rejected file sampled for context; clearly-labeled synthetic rejected applicants added for reject-inference/policy simulation.
...
- **Target `default_flag` — a default / severe-delinquency PROXY (labeled as such everywhere: outputs, `methodology_summary.json`, UI):** 1 = `Charged Off`, `Default`, `Late (31-120 days)`, `Does not meet the credit policy. Status:Charged Off`; 0 = `Fully Paid`. **Drop** `Current`, `In Grace Period`, `Late (16-30 days)`, and other in-progress/ambiguous statuses. Not presented as pure charge-off default. Exact definition recorded in `methodology_summary.json`.
- **Rejects:** used only for approval-funnel, policy, and reject-inference context. **Never** used for supervised PD training (rejected applicants have no observed repayment outcome).
